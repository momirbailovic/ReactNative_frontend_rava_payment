        ...
        return (
            <View style={styles.container}>
                <View style={styles.topContainer}>
                    <Text style={styles.imageTitle}>{images[index].title||defaultTitle}</Text>
                </View>
                <View style={styles.centerContainer}>
                    ...
                    >
                        {images.map((image, idx) => 
                            <Image key={idx} source={image.image} style={styles.centerImage} />
                        )}
                    </ScrollView>
                </View>
                    ...
                    <View style={styles.scrollContainer}>
                        <ScrollView ref="small" showsHorizontalScrollIndicator={false} horizontal style={styles.scroll}>
                            {images.map((image, idx) => <TouchableOpacity key={idx} onPress={() => this.changePage(idx)}>
                                <Image source={image.image} style={[styles.bottomImage, { borderWidth: index == idx ? 1.5 : 0 }]} />
                            </TouchableOpacity>)}
                        </ScrollView>
                    </View>
                </View>
